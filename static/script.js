...
let frames = [
  "/static/img/magnifier.png",
  "/static/img/machine.png",
  "/static/img/phone.png",
  "/static/img/bar_chart.png",
];
...
for (let i = 0; i < cards.length; i++) {
  ...
  cards[i].addEventListener(
    "click",
    function (event) {
      const panelTitle = this.closest('.panel').querySelector('h2').textContent;

      // Check if the clicked panel is in the panelUrls object
      if (panelTitle in panelUrls) {
        // Redirect to the specified URL
        window.location.href = panelUrls[panelTitle];
      }
    }
  );
}
...
gsap.ticker.add(() => {
  const dt = 1.0 - Math.pow(1.0 - speed, gsap.ticker.deltaRatio()) *2;
